...
export default function DateStaffSelection({ businessId, serviceId, serviceDuration, selectedService, onSelect }) {
  ...
  <div className="bg-gray-50 p-4 rounded-md space-y-4">
    <div className="flex justify-between items-center">
      <div>
        <h3 className="font-medium text-base">{selectedService === null || selectedService === void 0 ? void 0 : selectedService.title}</h3>
        <p className="text-sm text-gray-500">
          {formatDuration(serviceDuration)} - {selectedService === null || selectedService === void 0 ? void 0 : selectedService.price}€
        </p>
      </div>
      <div>
        <Select defaultValue="no_preference" value={(selectedStaff === null || selectedStaff === void 0 ? void 0 : selectedStaff.id) || "no_preference"} onValueChange={(value) => {
  ...
  <div className="border rounded-md">
    <div className="grid grid-cols-7 border-b">
      {weekDays.map((day) => (<div key={day.toISOString()} className="p-3 text-center border-r last:border-r-0">
    ...
    <div className="flex justify-between p-2 border-b bg-gray-50">
      <button onClick={() => setWeekStart(addDays(weekStart, -7))} className="p-1.5 rounded-md hover:bg-gray-100 transition-colors">
    ...
    <div className="grid grid-cols-7">
      {weekDays.map((day) => {
            ...
            return (<div key={dayStr} className="min-h-[400px] p-2 border-r last:border-r-0">
            {slots.map((slot) => (<button key={slot.time} className={`
                ...
                `} disabled={selectedStaff
                        ? !slot.availableStaff.some(s => s.id === selectedStaff.id)
                        : slot.availableStaff.length === 0} onClick={() => handleTimeSelect(day, slot)}>
                {slot.time}
              </button>))}
          </div>);
        })}
  ...
  {!loading && Object.values(availableSlots).every(slots => slots.length === 0) && (<div className="text-center text-gray-500 p-4">
      Aucun créneau disponible dans les 8 prochaines semaines.
      Veuillez nous contacter directement pour plus d'informations.
    </div>)}
    </div>);
}
//# sourceMappingURL=DateStaffSelection.jsx.map