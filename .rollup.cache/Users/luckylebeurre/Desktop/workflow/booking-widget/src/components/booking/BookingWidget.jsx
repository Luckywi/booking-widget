...
export default function BookingWidget({ businessId }) {
    ...
    if (loading) {
        return (<Card className="w-full max-w-3xl mx-auto p-4">
    ...
    {[1, 2, 3].map((number) => (<div key={number} className="flex flex-col items-center gap-2">
        <div className={`
    ...
    <div className="space-y-8">
      {serviceCategories.map((category) => (<div key={category.id} className="space-y-4">
          {/* Titre de catégorie */}
          ...
          <div className="space-y-3">
            {services
                    ...
                    .map((service) => (<div key={service.id} onClick={() => handleServiceSelect(service)} className={`
  ...
  {step === 3 && selectedService && selectedDateTime && selectedStaffMember && (<div className="space-y-4">
    <div className="flex justify-between items-center">
      <h2 className="text-xl font-semibold">Vos informations</h2>
      <button onClick={() => setStep(2)} className="text-sm text-gray-500 hover:text-black transition-colors duration-200 flex items-center gap-1">
        <ChevronLeft className="w-4 h-4"/>
        Modifier la date
      </button>
    </div>
    <ClientForm service={selectedService} dateTime={selectedDateTime} staff={selectedStaffMember} onSubmit={async (clientData) => {
                try {
...
//# sourceMappingURL=BookingWidget.jsx.map