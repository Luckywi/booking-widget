...
export default function ConfirmationPage() {
  ...
  <div className="space-y-2 text-sm">
    ...
    <p>
      ...
      <span className={`font-medium ${appointment.status === 'cancelled'
            ? 'text-red-600'
            : 'text-green-600'}`}>
        {appointment.status === 'cancelled' ? 'Annulé' : 'Confirmé'}
            ...
            {isAppointmentCancellable && (<div className="flex justify-center">
                <Button variant="destructive" onClick={handleCancelAppointment} disabled={cancelLoading}>
                  {cancelLoading ? 'Annulation...' : 'Annuler ce rendez-vous'}
                </Button>
              </div>)}
      
      {pastAppointments.length > 0 && (<div className="mt-8">
    ...
    <div className="space-y-4">
      ...
      {(showAllAppointments ? pastAppointments : pastAppointments.slice(0, 2)).map((apt) => (<div key={apt.id} className="p-3 border rounded-lg flex justify-between items-center hover:bg-gray-50">
          <div>
      ...
      {pastAppointments.length > 2 && (<button onClick={() => setShowAllAppointments(!showAllAppointments)} className="w-full mt-4 flex items-center justify-center gap-2 text-gray-500 hover:text-gray-700 text-sm py-2">
        <span>{showAllAppointments ? 'Voir moins' : 'Voir plus'}</span>
        {showAllAppointments ? (<ChevronUp className="w-4 h-4"/>) : (<ChevronDown className="w-4 h-4"/>)}
      </button>)}
    </div>
  </div>)}
      
            {appointment.status === 'confirmed' && (<div className="text-center text-sm text-gray-500">
            ...
            {appointment.status === 'confirmed' && (<div className="flex justify-center gap-4">
                <Link href="/">
            ...
            {appointment.status === 'cancelled' && (<div className="flex justify-center">
                <Button onClick={() => window.print()} variant="outline">
                  Imprimer
                </Button>
              </div>)}
